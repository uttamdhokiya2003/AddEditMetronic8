...
namespace AddEditMetronic8.DAL
{
    public class SEC_DALBase: DAL_Helper
	{
        ...
        public decimal? dbo_PR_SEC_User_Insert(string UserName, string Password, string FirstName, string LastName, string EmailAddress, string PhotoPath,DateTime? Created,DateTime? Modified)
        {
            try
            {
                SqlDatabase sqlDB = new SqlDatabase(ConnectionStr);
